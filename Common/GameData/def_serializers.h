...
void saveTo(const Parse_Effectiveness & target, const String &baseName, bool text_format=false);


constexpr const static uint32_t origins_i0_requiredCrc = 0x763EA46E;
bool loadFrom(BinStore *s,Parse_AllOrigins &target);
void saveTo(const Parse_AllOrigins & target, const String &baseName, bool text_format=false);


constexpr const static uint32_t pischedule_i0_requiredCrc = 0x27267F27;
bool loadFrom(BinStore *s,Parse_PI_Schedule &target);
void saveTo(const Parse_PI_Schedule & target, const String &baseName, bool text_format=false);
