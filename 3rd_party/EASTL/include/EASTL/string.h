    ...
    public:
       ...
       ~basic_string();
        ...
        static eastl::vector<this_type,allocator_type> split_str(view_type str, value_type separator, bool keepEmptyStrings = false)
        {
            ...
            for (const char* splitEnd = strStart; splitEnd != strEnd; ++splitEnd)
            {
                if (*splitEnd == separator)
                {
                    const ptrdiff_t splitLen = splitEnd - strStart;
                    if (splitLen > 0 || keepEmptyStrings)
                        ret.push_back(this_type(strStart, splitLen));
                    strStart = splitEnd + 1;
                }
            }

            const ptrdiff_t splitLen = strEnd - strStart;
            if (splitLen > 0 || keepEmptyStrings)
                ret.emplace_back(this_type(strStart, splitLen));
        ...
        {
            return split_str(*this, separator, keepEmptyStrings);
        }
