...
namespace eastl
{
...
#if EASTL_SEGS_EXTENSIONS
        const T& at(const Key& key,const T &default_val) const noexcept;
        const Key& key(const T &val,const Key &default_key) const noexcept;
        // Erase an element that matches the given key
        template<typename U, typename Compare2=eastl::less<void>>
...
#if EASTL_SEGS_EXTENSIONS
    template <typename Key, typename T, typename Compare, typename Allocator>
    inline const T& map<Key, T, Compare, Allocator>::at(const Key& key,const T &default_val) const noexcept
    {
    ...
    template <typename Key, typename T, typename Compare, typename Allocator>
    inline const Key& map<Key, T, Compare, Allocator>::key(const T &val,const Key &default_key) const noexcept
    {
